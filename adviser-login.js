      ...
      getDoc(documentRef).then((docSnap) => {
        if (docSnap.exists()) {
          ...
          if (userData.type === "student" || userData.type === "admin") {
            ...
            alert("You are not allowed to login as an adviser.");
            return;
          } else {
            ...
            setDoc(documentRef, {
              ...
              email: user.email,
              image: user.photoURL
            }, {merge: true}).then(() => window.location.href = "adviser-dashboard.html");
          }
        } else {
...
document.getElementById("adviser-login").addEventListener("click", signIn);