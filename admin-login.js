    ...
    .then((result) => {
      ...
      getDoc(documentRef).then((docSnap) => {
        if (docSnap.exists()) {
          ...
          if (userData.type === "faculty" || userData.type === "student") {
            // Adviser, faculty, or admin is logged in, prevent student login
            alert("You are not allowed to login as an admin.");
            return;
          } else {
            ...
            setDoc(documentRef, {
              ...
              email: user.email,
              image: user.photoURL
            }, {merge: true}).then(() => window.location.href = "admin-dashboard.html");
          }
        } else {
...
document.getElementById("admin-login").addEventListener("click", signIn);